...
export default ({vertices, edges, defaultEdgeAttrs, forces: forces0, showDetails}: Props) => {
  ...
  //const [props, setProps] = React.useState<Props>({vertices, edges, defaultEdgeAttrs, forces: forces0, showDetails})
  //const [nodesRef,setNodes] = React.useState<Array <JSX.Element>>(vertices.map(v => <EmbedVert v={v}/>))
    ...
    React.useEffect(() => {
      const cb = () => {
        ...
        d3.select<SVGSVGElement, unknown>(ref.current)
          ...
          .on('click',(_event,_data) => sendMessage(v.id))
          .call(
  ...
  async function sendMessage (msg : Message) {
    ...
    const newProps = await rs.call<Message, Props> ("sendMessage",msg)
    setNodes(newProps.vertices.map(v => <EmbedVert v={v}/>))
*/
  }
  return (
    <div ref={setRef}
